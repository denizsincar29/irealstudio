...
- **Alt** открывает нативное меню: **Файл** (Новая цифровка Ctrl+N / Открыть Ctrl+O /
  Сохранить Ctrl+S / Сохранить как / Экспорт в iReal Pro Ctrl+E / Выход),
...
## Slice 2 (форма «Новая цифровка» Ctrl+N) — что собрать и проверить

То же окно, что в slice 1. **Ctrl+N** (или Файл → Новая цифровка) открывает
модальный **wxDialog** «Новая цифровка» (нативная роль диалога — NVDA объявляет
заголовок и ведёт по контролам). Форма — калька python `_NewProjectDlg`
(dialogs.py:240): поля «Название» (по умолчанию My Progression), «Композитор»,
«Темп (BPM)» (спин 40–240, по умолчанию 120), «Тональность» (12 корней) +
«Лад» (мажор/минор), «Стиль» (все 50 стилей iReal, по умолчанию Medium Swing),
«Шаблон» (Без шаблона / Blues / AABA / ABAC / ABAB / ABCD), кнопки ОК / Отмена.

При ОК строится новая цифровка через `Doc::new_chart` (тот же путь, что python
`new_project` + `_apply_template`) и озвучивается «Новая цифровка: <название>»:
- Без шаблона — пустой документ (такты появляются при вводе аккордов, как в python).
- **Blues** — 12 пустых тактов без меток (в этом slice без выбора 16/24).
- AABA-семейство — скелет по 8 тактов на секцию с метками *A..*D (без вступления/
  коды и выбора длин секций — отдельный шаг).

Проверка на Windows: Ctrl+N открывает форму, Tab ходит по полям (NVDA читает
каждый контрол), стрелками раскрываются комбобоксы; ОК создаёт цифровку (сетка
перерисовывается, «секция A» слышна на F5 при шаблоне), Отмена ничего не меняет.

## Slice 3 (открыть/сохранить .ips) — что собрать и проверить

То же окно. Меню **Файл** пополнено пунктами **Открыть… (Ctrl+O)**,
**Сохранить (Ctrl+S)**, **Сохранить как…**. Файл `.ips` — ровно JSON цифровки
(`ChordProgression.to_json`), как в python-версии irealstudio; русские/латинские
названия и все аккорды переживают round-trip без потерь.

- **Ctrl+S** — сохранить. Если цифровка уже связана с файлом (открыта или
  сохранена ранее) — пишет прямо в него, озвучка «Сохранено: <имя файла>».
  Если файла ещё нет (новая цифровка по Ctrl+N) — как python `save()`: открывает
  диалог «Сохранить как» с именем по умолчанию `Название.ips`
  (пробелы → `_`). **Сохранить как…** всегда открывает этот диалог
  (`wxFD_SAVE | wxFD_OVERWRITE_PROMPT` — предупреждает о перезаписи).
- **Ctrl+O** — диалог открытия (`wxFD_OPEN | wxFD_FILE_MUST_EXIST`); после
  загрузки курсор на такте 1, сетка перерисована, озвучка «Открыто: <название>».
  Дальше Ctrl+S пишет в этот же файл.
- Ошибки (нет файла, битый JSON) — озвучка «Не удалось открыть/сохранить: …»,
  документ не трогается.

Проверка на Windows: Ctrl+N → новая цифровка (напр. название «My Song») →
Ctrl+S → в диалоге уже стоит `My_Song.ips`; выбрать путь и сохранить → закрыть
и снова Ctrl+O → открыть этот файл → озвучка «Открыто: My Song», стрелки ходят
по тактам; F6 озвучивает всю цифровку.

## Slice 4 (экспорт в iReal Pro Ctrl+E) — что собрать и проверить

Экспорт — как python `export_ireal` (app_io.py:413): по текущей цифровке
строится `irealb://` URL (с BPM, тот же кодек, что сверен побайтово в core), и
сохраняется **HTML** с авто-редиректом на него — откройте файл в браузере на
устройстве с iReal Pro, и цифровка импортируется сама. Расширение `.txt` —
отладочный вариант: сырой не-URL-encoded `irealbook://` URL без HTML.

- Меню **Файл → Экспорт в iReal Pro… (Ctrl+E)**. Диалог сохранения предлагает
  `Название.html` (пробелы/запрещённые символы вычищены как `_safe_filename`)
  в папке текущего файла `.ips`, если он есть.
- Чистая логика — в lib (`safe_file_base`, `export_ireal_html`), HTML сверен
  с python побайтово (тест в контейнере).

Проверка на Windows: открыть/создать цифровку → Ctrl+E → сохранить `.html` →
открыть файл браузером на устройстве с iReal Pro (или, если на Windows стоит
iReal Pro и зарегистрирован протокол, просто открыть файл) → цифровка с темпом
появляется в iReal Pro. `.txt`-экспорт отдаёт читаемый irealbook-URL.
