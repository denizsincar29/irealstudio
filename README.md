...
Статус: **этап 1 — порт чистого ядра** (кодек irealb готов и сверен с python-эталоном; гармония — в работе). GUI/wxDragon — этап 2.
...
`core` держит golden-тесты: `core/tests/golden_songs.rs` (автогенерится из
`irealstudio/irealb.py` на реальных песнях iReal Pro) сверяет decode + обратный
encode. Любое расхождение порта с эталоном — красный тест.
